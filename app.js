...
function home() {
  ...
  app.innerHTML = `<section class="hero container"><div class="hero-intro"><div><span class="eyebrow">LIVE FROM NEWSAPI</span><h1>Technology,<br><em>decoded.</em></h1></div><span class="date-stamp">${featured.date}</span></div><div class="feature-grid"><article class="feature-card"><img src="${featured.img}" alt="${featured.title}"><div class="feature-copy">${tag(featured.cat)}<h2><a href="#article/${featured.id}">${featured.title}</a></h2><p>${featured.desc}</p>${meta(featured)}</div></article><div class="supporting">${rest.slice(0, 3).map(supportCard).join("")}</div></div></section><div class="container content-layout"><section class="latest"><div class="section-head"><h2>Latest news</h2><a class="view-all" href="#latest">View all</a></div><div class="latest-grid">${articles.slice(1, 6).map(newsCard).join("")}</div></section>${trending()}</div><section class="category-strip"><div class="container"><div class="section-head"><h2>Explore the beat</h2><span class="eyebrow">BY CATEGORY</span></div><div class="category-grid">${["AI", "Programming", "Software", "Cybersecurity", "Gadgets", "Startups"].map(miniSection).join("")}</div></div></section><div class="container">${newsletter()}</div>`;
}
